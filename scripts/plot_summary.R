...
# nova_palette <- c("#4ED0CD", "#FFD966")
# dark_aqua <- colorspace::darken("#4ED0CD", amount = 0.5, space = "HLS")
# dark_yellow <- colorspace::darken("#FFD966", amount = 0.35, space = "HLS")
...
# --- Pivot and clean ---
pivot_and_clean <- function(df, groupers, value_prefix, name_col, value_col = "value") {
  pivot_cols <- names(df)[startsWith(names(df), value_prefix)]
...
# Function to generate all combinations of group variables and run summaries
run_all_group_summaries <- function(data, summarization_var, group_var_list) {
  # Generate all possible combinations of group variables (including single variables and multiple)
  group_combinations <- lapply(1:length(group_var_list), function(i) {
    combn(group_var_list, i, simplify = FALSE)
  }) %>% unlist(recursive = FALSE)

  # Run summaries for each combination
  summary_dataframes <- lapply(group_combinations, function(group_vars) {
    summary <- novahelpers::run_summary(data, group_vars, summarization_var)
    # Assign a name to each summary based on the grouping variables
    group_name <- paste(group_vars, collapse = "_")
    summary <- summary %>%
      mutate(grouping_vars = group_name) # Add a column for the grouping combination
    #
    summary <- summary %>%
      filter(n != 0)
  ...
  # Name each dataframe in the list based on the grouping combination
...
get_descriptive_result <- function(results, cont_var, group_vars, delim = "__") {
...
make_name <- function(project = NULL, subset = NULL, period = NULL, demo = NULL, y_var = NULL,
                      x_var = NULL, fill_var = NULL, facet_col = NULL, facet_row = NULL,
                      object = NULL, specifier1 = NULL, specifier2 = NULL, specifier3 = NULL) {
  paste(c(project, subset, period, demo, y_var, x_var, fill_var, facet_col, facet_row, object, specifier1, specifier2, specifier3), collapse = "_") |>
    stringr::str_remove("_$") # remove trailing underscore if specifier is NULL
}
...
#   #     filter(if_all(all_of(na_filter_vars), ~ !is.na(.x)))
...
#
#   if (!is.null(na_filter_vars)) {
#     # Filter only if vars exist
#     vars_in_long <- intersect(na_filter_vars, names(long_data_filtered))
#     if (length(vars_in_long) > 0) {
#       long_data_filtered <- long_data_filtered %>%
#         filter(if_all(all_of(vars_in_long), ~ !is.na(.x)))
#     }
#
#     vars_in_summary <- intersect(na_filter_vars, names(summary_filtered))
#
#     if (length(vars_in_summary) > 0) {
#       summary_filtered <- summary_filtered %>%
#         filter(if_all(all_of(vars_in_summary), ~ !is.na(.x)))
#     }
#   }
#
#
#   # Determine which individuals remain
#   if (!is.null(id_var)) {
#     individuals_retained <- long_data_filtered %>%
#       distinct(.data[[id_var]])
#
#     # filter individual_data to only retained individuals
#     individual_data_filtered <- individual_data %>%
#       semi_join(individuals_retained, by = id_var)
#
#     initial_ids <- nrow(individual_data)
...
#   }
#
#   initial_obs <- nrow(long_data)
#   retained_obs <- nrow(long_data_filtered)
#
#   report <- tibble(
#     initial_individuals = initial_ids,
#     retained_individuals = retained_ids,
#     initial_observations = initial_obs,
#     retained_observations = retained_obs,
#     retained_groups = nrow(allowed_combinations)
#   )
#
#   list(
...
get_plot_size <- function(raw_data, x_var, fill_var = NULL,
                          facet_row_var = NULL, facet_col_var = NULL,
                          base_height = 6, base_width = 8,
                          x_factor = 0.2, fill_factor = 0.5,
                          facet_row_factor = 2.5, facet_col_factor = 2.0,
                          max_height = 14, max_width = 20) {
  n_x_levels <- if (!is.null(x_var)) length(unique(raw_data[[x_var]])) else 1
  n_fill_levels <- if (!is.null(fill_var)) length(unique(raw_data[[fill_var]])) else 1
  n_row_facets <- if (!is.null(facet_row_var)) length(unique(raw_data[[facet_row_var]])) else 1
  n_col_facets <- if (!is.null(facet_col_var)) length(unique(raw_data[[facet_col_var]])) else 1

  width <- base_width + (n_x_levels * x_factor) + (n_col_facets - 1) * facet_col_factor
  height <- base_height + (n_row_facets - 1) * facet_row_factor
...
plot_summary <- function(raw_data, summary_data,
                         ...
                         facet_row_var = NULL,
                         facet_col_var = NULL,
                         slabjust = 2, dots = FALSE, scaling = 0.4, flipped = FALSE,
                         mean_nudge_x = -0.2, mean_nudge_y = 0.5, stagger = 0.5,
                         base_size = 16, axis_text_rel = 1.5, axis_title_rel = 2,
                         label_text_size = 5, n_label_rel = 0.8,
                         n_nudge = 0.5, dotsize = .4,
...
#   facet_col_lab <- if (!is.null(facet_col_var)) var_dict$label[var_dict$var_name == facet_col_var] else NULL
  ...
  p <- ggplot(raw_data, aes(x = .data[[x_var]], y = .data[[y_var]])) +
    ...
    my_theme +
    scale_fill_manual(values = base_palette, labels = function(x) stringr::str_wrap(x, width = 10)) +
  ...
  p <- p +
    ggdist::stat_slab(
      alpha = 0.4, adjust = slabjust, side = direction, scale = scaling,
...
plot_summary_wrapper <- function(
    ...
    width = 8) {
  # --- Create plot name ---
  fig_name <- paste(c(cont_focus, group_focus), collapse = "__")
...
#
# flipper_summaries <- run_all_group_summaries(df, summarization_var, group_var_list)
#
...
# plot_summary(df, flipper_summaries[["species"]], y_var = "flipper_length_mm", "species")
# plot_summary(df, flipper_summaries[["species_sex"]], y_var = "flipper_length_mm", "species", "sex")
# plot_summary(df, flipper_summaries[["species_sex_island"]], y_var = "flipper_length_mm", "species", "sex", "island", flipped = TRUE, scaling = .6, dots = TRUE, dotsize = .6)
#
#
#
#
...
#       ),
#       alpha = .5,
#       # position = ggdist::position_dodgejust(),
#       # position = position_dodge(),
#       # position = position_dodge(width = .5),
#       position = position_nudge(y = -.5),
#       # position = position_dodge2(),
#       shape = 19, size = 2, data = na.omit(penguins), scale = 0.5
#     ) +
